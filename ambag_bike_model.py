...
def main():
    ...
    parser.add_argument(
        "--name", dest="name", action="store", choices=model.list_steps()
    )
    args = parser.parse_args()

    ambag_base = model.Scenario(
        name="base", config="ambag_example/configs", data="ambag_example/base"
    )

    ambag_build = model.Scenario(
        name="build", config="ambag_example/configs", data="ambag_example/build"
    )
...
def preprocess_network(net):
    ...
    for a in net.adjacency:
        for b in net.adjacency[a]:
            ...
            net.set_link_attribute_value(
                (a, b), "d0", distance * (bike_class == 0 and lanes > 0)
            )
            ...
            net.set_link_attribute_value(
                (a, b), "dw", distance * (bike_class == 0 and lanes == 0)
            )
            net.set_link_attribute_value(
                (a, b), "bike_exclude", 1 * (link_type in ["FREEWAY"])
            )
            net.set_link_attribute_value(
                (a, b), "auto_permit", 1 * (link_type not in ["BIKE", "PATH"])
            )
            net.set_link_attribute_value(
                (a, b), "dloc", distance * (fhwa_fc in [19, 9])
            )
            net.set_link_attribute_value(
                (a, b), "dcol", distance * (fhwa_fc in [7, 8, 16, 17])
            )
            net.set_link_attribute_value(
                (a, b), "dart", distance * (fhwa_fc in [1, 2, 6, 11, 12, 14, 77])
            )
            net.set_link_attribute_value(
                (a, b), "dne3loc", distance * (fhwa_fc in [19, 9]) * (bike_class != 3)
            )
            net.set_link_attribute_value(
                (a, b),
                "dne2art",
                distance * (fhwa_fc in [1, 2, 6, 11, 12, 14, 77]) * (bike_class != 2),
            )

    # add new dual (link-to-link) attribute columns
    net.add_dual_attribute(
        "thru_centroid"
    )  # from centroid connector to centroid connector
    ...
    for link1 in net.dual:
        for link2 in net.dual[link1]:
            ...
            net.set_dual_attribute_value(
                link1, link2, "thru_centroid", 1 * (traversal_type == 0)
            )
            net.set_dual_attribute_value(
                link1, link2, "u_turn", 1 * (traversal_type == 3)
            )
            net.set_dual_attribute_value(
                link1, link2, "l_turn", 1 * (traversal_type in [5, 7, 10, 13])
            )
            net.set_dual_attribute_value(
                link1, link2, "r_turn", 1 * (traversal_type in [4, 6, 9, 11])
            )
            net.set_dual_attribute_value(
                link1,
                link2,
                "turn",
                1 * (traversal_type in [3, 4, 5, 6, 7, 9, 10, 11, 13]),
            )
            net.set_dual_attribute_value(
                link1, link2, "thru_intersec", 1 * (traversal_type in [8, 12])
            )
            net.set_dual_attribute_value(
                link1, link2, "thru_junction", 1 * (traversal_type == 14)
            )
