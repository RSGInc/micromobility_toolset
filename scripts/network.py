...
class Network():
	...
	def __init__(self,network_config,sqlite_file):
		...
		complete = self.is_network_complete()
		if not complete:
			raise Exception('network links and nodes are not complete')
	...
	def read_links_from_sqlite(self,file_name,table_name,from_name,to_name,attributes_by_direction):
		...
		## attributes_by_direction = dictionary of { name in network data structure : ( column name for ab direction, column name for ba direction) }
		
		# put desired attribute names into network data structure
		self.adjacency_names = attributes_by_direction.keys()
		...
		while True:
			...
			if row is None:
				...
				for k in attributes_by_direction.keys():
					# get values for equivalent database column names
					ab_attribute_values.append( row[row.keys().index(attributes_by_direction[k][0])] )
					ba_attribute_values.append( row[row.keys().index(attributes_by_direction[k][1])] )
				
				# get a node and b node
				a = row[row.keys().index(from_name)]
				b = row[row.keys().index(to_name)]
	...
	def read_nodes_from_sqlite(self,file_name,table_name,node_name,attributes):
		...
		# put desired attribute names into network data structure
		self.node_names = attributes.keys()
		...
		while True:
			...
			if row is None:
				...
				# set up attribute value list
				attribute_values = []
				
				# loop over desired attribute names
				for k in attributes.keys():
					# get values for equivalent database column names
					attribute_values.append( row[row.keys().index(attributes[k])] )
				
				# get node id
				n = row[row.keys().index(node_name)]
				
				# insert into node dictionary and/or set attribute values
				self.nodes[n] = attribute_values
	...
	def is_network_complete(self):
		"""check to see that all nodes have edges and nodes for all edges have defined attributes, boolean"""
		
		# get ready to return true if no errors found
		return_value = True
		
		for n in self.nodes:
			if n not in self.adjacency or len(self.nodes[n]) == 0:
				# if node does not have edge or node does not have attributes get ready to return false
				return_value = False
				break
		
		# return true if network is good and false otherwise
		return return_value
	...
	def traversal_type(self,edge1,edge2,consideration_field=None):
		...
		for neighbor in self.adjacency[edge1[1]]:
			...
			if consideration_flag and not ( self.is_centroid_connector((edge1[1],neighbor)) or neighbor == edge1[0] ):
				...
				min_angle= min(i for i in (min_angle,current_angle) if i is not None)
				max_angle = max(i for i in (max_angle,current_angle) if i is not None)
